...
void sendOpskriftTilServer(Opskrift opskrift) {
  // Server URL
  String url = "http://server-kopi.onrender.com/opskrifter";
  
  // Opret JSON objekt til at holde opskriftens data
  JSONObject json = new JSONObject();
  
  // Tilføj opskriftens grundlæggende data
  json.setString("titel", opskrift.titel);
  json.setString("kategori", opskrift.kategori);
  json.setString("url", opskrift.link);
  json.setString("produkttype", opskrift.produktType);
  
  // Tilføj garn som et JSONArray
  JSONArray garnArray = new JSONArray();
  for (int i = 0; i < opskrift.krævneGarn.size(); i++) {
    garnArray.setString(i, opskrift.krævneGarn.get(i));
  }
  json.setJSONArray("garn", garnArray);
  

  //// Håndter billedet, hvis det findes
  //if (opskrift.billede != null) {
  //  // Konverter billedet til base64 for at sende det over netværket
  //  String base64Image = billedeBase64(opskrift.billede);
  //  json.setString("image", base64Image);
  //}
  
  //// Håndter PDF, hvis det findes
  //if (selectedPdfPath != null && !selectedPdfPath.isEmpty()) {
  //  String base64Pdf = filBase64(selectedPdfPath);
  //  json.setString("pdf", base64Pdf);
  //  json.setString("pdfName", selectedPdfName);
  //}
  ...
  // Hent responsen som en String (f.eks. HTTP-statuskoden kan være en del af headeren)
  String responseContent = post.getContent();
  String responseCode = post.getHeader("Status"); // Hent statuskode fra header
  
  // Hvis statuskoden findes
  if (responseCode != null) {
    try {
      int responseCodeInt = Integer.parseInt(responseCode); // Konverter til int
      println("Statuskode: " + responseCodeInt);
      
      // Sammenlign med de ønskede koder
      if (responseCodeInt == 200 || responseCodeInt == 201) {
        println("✅ Opskrift sendt til serveren med succes!");
        
        // Tilføj opskriften til den lokale liste over hentede opskrifter, hvis den ikke allerede er der
        boolean findes = false;
        for (Opskrift o : alleOpskrifter) {
          if (o.titel.equals(opskrift.titel)) {
            findes = true;
            break;
          }
        }
        ...
        if (!findes) {
          alleOpskrifter.add(opskrift);
          visteOpskrifter.add(opskrift);
        }
        
      } else {
        println("⚠️ Fejl ved sending af opskrift: " + responseCodeInt);
        println("Fejlbesked: " + responseContent);
      }
    } catch (NumberFormatException e) {
      println("⚠️ Fejl: Ikke en gyldig statuskode.");
    }
  } else {
    println("⚠️ Fejl: Ingen statuskode modtaget.");
  }
}
